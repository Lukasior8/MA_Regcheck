...
## ## test, retest korrelation Cohens Kappa
...
## erstelle einen Überblick über die kappas
...
# Schritt 1: Berechne den minimalen und maximalen Kappa-Wert für alle Kappa-Spalten
min_kappa <- min(kappa_summary_overview %>% select(starts_with("Kappa")), na.rm = TRUE)
max_kappa <- max(kappa_summary_overview %>% select(starts_with("Kappa")), na.rm = TRUE)


kappa_summary_overview %>%
  kbl() %>%
  kable_paper("striped", full_width = F) %>%
  column_spec(3, background = spec_color(kappa_summary_overview$Kappa_pre, end = 0.9, option = "D", scale_from = c(min_kappa, max_kappa))) %>%
  column_spec(5, background = spec_color(kappa_summary_overview$Kappa_pap, end = 0.9, option = "D", scale_from = c(min_kappa, max_kappa))) %>%
  column_spec(7, background = spec_color(kappa_summary_overview$Kappa_match, end = 0.9, option = "D", scale_from = c(min_kappa, max_kappa)))
...
formattable(kappa_summary_overview, list(
  Kappa_pre = color_tile("darkred", "darkgreen"),  # Kappa_pre-Spalte von dunkelrot bis dunkelgrün
  Kappa_pap = color_tile("darkred", "darkgreen"),  # Kappa_pap-Spalte von dunkelrot bis dunkelgrün
  Kappa_match = color_tile("darkred", "darkgreen") # Kappa_match-Spalte von dunkelrot bis dunkelgrün
))
