...
retest_kappa_summary <- rbind(retest_kappa_extraction, retest_kappa_comparison)
...
## Kappa interrater gpt zu olmo  per category 3. ansatz inkl sensitivity,specifity precision per item
```{r}
...
valid_gpt_columns_per_item_v3 <- valid_gpt_columns_per_item_v3[!valid_gpt_columns_per_item_v3 %in% excluded_items]
valid_olmo_columns_per_item_v3 <- valid_olmo_columns_per_item_v3[!valid_olmo_columns_per_item_v3 %in% gsub("_gpt$", "_olmo", excluded_items)]
...
for(i in 1:length(valid_gpt_columns_per_item_v3)) {
  ...
  confusion_matrix <- table(combined_data_per_item_v3$GPT, combined_data_per_item_v3$OLMO)
...
write.xlsx(kappa_results_per_item_v3, "../Data/processed/kappa_results_per_item_v3.xlsx")
...
# mergen von data_protoype und retest_kappa summary
```{r}

# Anzeigen der einzigartigen Werte von 'Column' in retest_kappa_summary
unique(retest_kappa_summary$Column)
...
anti_join(data_prototype, retest_kappa_summary, by = c("Variable_clean" = "Column"))

# Durchführung des Merges nach Überprüfung
data_retest_prototype <- retest_kappa_summary %>%
