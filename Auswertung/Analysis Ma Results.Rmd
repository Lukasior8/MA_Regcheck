...
The following R code was developed with the assistance of AI tools. Specifically, the ChatGPT-4o model was employed to optimise the code, provide solutions, and implement adjustments. The code segments were iteratively refined and reviewed to ensure they performed the intended functions. Additionally, key results were recalculated using  tools like Jamovi to verify their accuracy.

The use of ChatGPT offered significant advantages, particularly in terms of speed and adaptability in generating code. It served as a valuable resource by translating human language into programming language. However, several challenges arose during this process. Firstly, ChatGPT occasionally generated incorrect or non-functional code. Secondly, its solutions often involved more complex or less efficient code variants, which sometimes introduced additional variables or unnecessary nesting. This approach frequently disregarded programming best practices, thereby reducing the clarity and transparency of the code.It should be noted, however, that similar issues—such as reduced code quality and comprehensibility—can also arise when using examples from online forums. 
As such, the ultimate responsibility for the correctness, completeness, and application of the code rests solely with the author.


## Prompt examples


Example 1: Create a Dataset with Prototypical Responses
Task:
Help me write R code to create a new dataset, data_prototype, based on data_gpt_olmo_processed. The new dataset should have:
1.	A column containing the variable names from data_gpt_olmo_processed.
2.	A column containing the most frequent (prototypical) response for each column in data_gpt_olmo_processed.
3.	A column showing the percentage of rows containing this most frequent response.

________________________________________
Example 2: Merge Prototype with Retest Data
Task:
Create a new dataset, data_retest_prototype, by merging retest_merged_data with data_prototype.
•	Match the Column column from retest_merged_data with the Variable column in data_prototype.
•	The issue: In retest_merged_data, variable names end with _comp, whereas in data_prototype, they end with _gpt. Variables ending with _olmo in data_prototype can be ignored.

________________________________________
Example 3: Include Confidence Interval and Round Results
Task:
Add a confidence interval to the code and round all results to two decimal places. Ensure the final dataset reflects these adjustments.

________________________________________
Example 4: Calculate Kappa per Item
Task:
Help me adjust the R code to calculate Cohen's Kappa for each individual column in data_gpt_olmo_processed that ends with _gpt or _olmo.
•	Create a new dataset named kappa_per_item_v3.
•	Include additional measures (e.g., p-value) for each column in the new dataset.
•	Rename all variable and function names to include the suffix _per_item_v3 to avoid confusion.

________________________________________
Example 5: Step-by-Step Code Explanation
Task:
Explain the following code line by line. Describe what each line does and why it is necessary:...
...
# Step 2: Get the number of columns (items/categories)
num_columns <- ncol(comp_columns)

# Step 3: Get the number of unique psps (rows)
num_psps <- nrow(comparison_data_processed)

# Step 4: Create a new dataset that counts the occurrences of each category across all "_comp" columns
...
# Step 5: Add percentages as a second row
...
# Step 6: Combine the two with row labels
...
# Step 7: Print the combined table with both counts and percentages
print(data_category_counts_total)

# Step 8: Print the number of items (columns) and psps (rows)
cat("Number of items (categories):", num_columns, "\n")
cat("Number of psps (rows):", num_psps, "\n")
...
## Sensitivität und Spezifität und accuracy für jede Klasse (One-vs-Rest)
...
# Berechnung der Konfidenzintervalle für die Y-Klasse
ci_sensitivity_Y <- wilson_ci(TP_Y, TP_Y + FN_Y)
ci_specificity_Y <- wilson_ci(TN_Y, TN_Y + FP_Y)
ci_precision_Y <- wilson_ci(TP_Y, TP_Y + FP_Y)

# Berechnung der Konfidenzintervalle für die N-Klasse
ci_sensitivity_N <- wilson_ci(TP_N, TP_N + FN_N)
ci_specificity_N <- wilson_ci(TN_N, TN_N + FP_N)
ci_precision_N <- wilson_ci(TP_N, TP_N + FP_N)

# Berechnung der Konfidenzintervalle für die X-Klasse
ci_sensitivity_X <- wilson_ci(TP_X, TP_X + FN_X)
ci_specificity_X <- wilson_ci(TN_X, TN_X + FP_X)
ci_precision_X <- wilson_ci(TP_X, TP_X + FP_X)

# Ergebnisse in einer Tabelle mit Konfidenzintervallen anzeigen
results_with_ci <- data.frame(
  Class = c("Y", "N", "X"),
  Sensitivity = round(c(sensitivity_Y, sensitivity_N, sensitivity_X), 2),
  Sensitivity_CI = paste0("[", round(c(ci_sensitivity_Y[1], ci_sensitivity_N[1], ci_sensitivity_X[1]), 2), 
                          ", ", round(c(ci_sensitivity_Y[2], ci_sensitivity_N[2], ci_sensitivity_X[2]), 2), "]"),
  Specificity = round(c(specificity_Y, specificity_N, specificity_X), 2),
  Specificity_CI = paste0("[", round(c(ci_specificity_Y[1], ci_specificity_N[1], ci_specificity_X[1]), 2), 
                          ", ", round(c(ci_specificity_Y[2], ci_specificity_N[2], ci_specificity_X[2]), 2), "]"),
  Precision = round(c(precision_Y, precision_N, precision_X), 2),
  Precision_CI = paste0("[", round(c(ci_precision_Y[1], ci_precision_N[1], ci_precision_X[1]), 2), 
                        ", ", round(c(ci_precision_Y[2], ci_precision_N[2], ci_precision_X[2]), 2), "]")
)
...
##  negative predictive value für jede Klasse

```{r}
print(confusion_matrix)
n_total <- sum(confusion_matrix)

#Berechnung des Wilson-Konfidenzintervalls
wilson_ci <- function(successes, total, conf_level = 0.95) {
  z <- qnorm(1 - (1 - conf_level) / 2)
  phat <- successes / total
  denominator <- 1 + (z^2 / total)
...
# Berechnung für jede Klasse (Y, N, X)
calculate_metrics <- function(confusion_matrix, actual_class, predicted_class) {
  TP <- confusion_matrix[actual_class, predicted_class]
  FN <- sum(confusion_matrix[actual_class, ]) - TP
  FP <- sum(confusion_matrix[, predicted_class]) - TP
  ...
  ci_sensitivity <- wilson_ci(TP, TP + FN)
  ci_specificity <- wilson_ci(TN, TN + FP)
  ci_precision <- wilson_ci(TP, TP + FP)
...
# Ergebnisse für die Klassen berechnen
metrics_Y <- calculate_metrics(confusion_matrix, "Actual_Y", "Predicted_Y")
metrics_N <- calculate_metrics(confusion_matrix, "Actual_N", "Predicted_N")
metrics_X <- calculate_metrics(confusion_matrix, "Actual_X", "Predicted_X")

# Ergebnisse in einer Tabelle mit Konfidenzintervallen anzeigen
results_with_ci <- data.frame(
  Class = c("Y", "N", "X"),
  Sensitivity = round(c(metrics_Y$sensitivity, metrics_N$sensitivity, metrics_X$sensitivity), 2),
  Sensitivity_CI = paste0("[", round(c(metrics_Y$ci_sensitivity[1], metrics_N$ci_sensitivity[1], metrics_X$ci_sensitivity[1]), 2), 
                          ", ", round(c(metrics_Y$ci_sensitivity[2], metrics_N$ci_sensitivity[2], metrics_X$ci_sensitivity[2]), 2), "]"),
  Specificity = round(c(metrics_Y$specificity, metrics_N$specificity, metrics_X$specificity), 2),
  Specificity_CI = paste0("[", round(c(metrics_Y$ci_specificity[1], metrics_N$ci_specificity[1], metrics_X$ci_specificity[1]), 2), 
                          ", ", round(c(metrics_Y$ci_specificity[2], metrics_N$ci_specificity[2], metrics_X$ci_specificity[2]), 2), "]"),
  Precision = round(c(metrics_Y$precision, metrics_N$precision, metrics_X$precision), 2),
  Precision_CI = paste0("[", round(c(metrics_Y$ci_precision[1], metrics_N$ci_precision[1], metrics_X$ci_precision[1]), 2), 
                        ", ", round(c(metrics_Y$ci_precision[2], metrics_N$ci_precision[2], metrics_X$ci_precision[2]), 2), "]"),
  NPV = round(c(metrics_Y$npv, metrics_N$npv, metrics_X$npv), 2),
  NPV_CI = paste0("[", round(c(metrics_Y$ci_npv[1], metrics_N$ci_npv[1], metrics_X$ci_npv[1]), 2), 
                  ", ", round(c(metrics_Y$ci_npv[2], metrics_N$ci_npv[2], metrics_X$ci_npv[2]), 2), "]")
)
...
## test, retest korrelation Cohens Kappa per category v1
...
## test, retest korrelation Cohens Kappa per category vX anderer Datensatz
```{r}
# Step 1: Filter the dataset into test and retest subsets
test_data <- data_gpt_olmo_processed %>% filter(test_retest == "Test")
retest_data <- data_gpt_olmo_processed %>% filter(test_retest == "RETEST")

# Step 2: Match test and retest based on 'psp' (so this only contains the 30 test-retest Psp rows)
retest_merged_data <- merge(test_data, retest_data, by = "psp", suffixes = c("_test", "_retest"))

# Step 3: Get all columns ending with '_gpt'
gpt_columns <- grep("_gpt$", names(data_gpt_olmo_processed), value = TRUE)

# Step 4: Loop through each '_gpt' column and calculate Cohen's Kappa
retest_kappa_results <- lapply(gpt_columns, function(col_name) {
  # Cohen's Kappa requires the two sets of categorical data (Test and RETEST)
  test_col <- retest_merged_data[[paste0(col_name, "_test")]]
  retest_col <- retest_merged_data[[paste0(col_name, "_retest")]]
...
# Combine the results into a single data frame
retest_kappa_summary_vX <- do.call(rbind, retest_kappa_results)

# Step 5: Display the results
print(retest_kappa_summary_vX)




#write to disk for jamovi
write.xlsx(retest_kappa_summary_vX, file = "../Data/processed/retest_kappa_summary_vX.xlsx")

```



## test, retest korrelation Cohens Kappa per category v2 mit zusätzlich z wert, agreement % , p wert
```{r}
...
retest_kappa_summary_v2 <- do.call(rbind, retest_kappa_results)

# Step 6: Display the results
print(retest_kappa_summary_v2)
...
## test, retest korrelation Cohens Kappa per category v3
```{r}
# Step 1: Filter the dataset into test and retest subsets
test_data <- comparison_data_processed %>% filter(test_retest == "Test")
retest_data <- comparison_data_processed %>% filter(test_retest == "RETEST")

# Step 2: Merge test and retest data based on 'psp' to only include the 30 test-retest psp rows
retest_merged_data <- merge(test_data, retest_data, by = "psp", suffixes = c("_test", "_retest"))
...
retest_kappa_summary_v3 <- data.frame(
  Variable = comp_columns,
  Kappa = NA,
  P_Value = NA,
  Z_Value = NA,
...
# Step 5: Loop through each '_comp' column and calculate Cohen's Kappa without adjustments for high percent agreement
for (i in 1:length(comp_columns)) {
  col_name <- comp_columns[i]
  
  # Extract the test and retest columns for the current '_comp' item
  test_col <- retest_merged_data[[paste0(col_name, "_test")]]
  retest_col <- retest_merged_data[[paste0(col_name, "_retest")]]
  
  # Check if there are non-NA data in both columns
  if (!all(is.na(test_col)) && !all(is.na(retest_col))) {
    # Remove rows with NA values for this comparison
    valid_data <- retest_merged_data[!is.na(test_col) & !is.na(retest_col), ]
    
    # Calculate Cohen's Kappa using the 'irr' package without any special adjustments
    kappa_result <- irr::kappa2(data.frame(valid_data[[paste0(col_name, "_test")]], 
                                           valid_data[[paste0(col_name, "_retest")]]), "unweighted")
    
    # Extract kappa value and p-value
    kappa_value <- kappa_result$value
    p_value <- kappa_result$p.value
    agreement_percentage <- mean(valid_data[[paste0(col_name, "_test")]] == 
                                 valid_data[[paste0(col_name, "_retest")]]) * 100
    
    # Calculate expected agreement (p_e)
    agreement_table <- table(valid_data[[paste0(col_name, "_test")]], valid_data[[paste0(col_name, "_retest")]])
    n <- sum(agreement_table)  # Total number of pairs
    p_e <- sum(rowSums(agreement_table) * colSums(agreement_table)) / (n^2)
    
    # Calculate observed agreement (p_0)
    p_0 <- agreement_percentage / 100
    
    # Calculate standard error (SE) and z-value if p_e < 1
    if (p_e < 1) {
      se_kappa <- sqrt((p_0 * (1 - p_0)) / (n * (1 - p_e)^2))
      z_value <- kappa_value / se_kappa
    } else {
    ...
    retest_kappa_summary_v3[i, ] <- c(col_name, kappa_value, p_value, z_value, agreement_percentage)
  }
}

# Step 6: Display the results
print(retest_kappa_summary_v3)
...
```{r}
# Berechnung des Durchschnitts der Spalte 'Kappa' und Rundung auf 2 Nachkommastellen
mean_kappa <- round(mean(retest_kappa_summary$Kappa, na.rm = TRUE), 2)

# Berechnung der Standardabweichung der Spalte 'Kappa' und Rundung auf 2 Nachkommastellen
sd_kappa <- round(sd(retest_kappa_summary$Kappa, na.rm = TRUE), 2)

# Berechnung des Bereichs (Range) der Spalte 'Kappa' und Rundung auf 2 Nachkommastellen
range_kappa <- round(range(retest_kappa_summary$Kappa, na.rm = TRUE), 2)

# Ausgabe der Ergebnisse
cat("Durchschnittlicher Kappa-Wert:", mean_kappa, "\n")
cat("Standardabweichung des Kappa-Werts:", sd_kappa, "\n")
cat("Bereich des Kappa-Werts:", range_kappa[1], "-", range_kappa[2], "\n")
...
```
weighted macht hier keinen sinn...
...
## test retest nur extraction nurcomparison...
```{r}
...
# Step 3: Define columns for extraction and comparison
extract_columns <- grep("^(c_pre|pre|c_pap|pap).*_comp$", names(comparison_data_processed), value = TRUE)
comp_columns <- grep("^(c_match|match).*_comp$", names(comparison_data_processed), value = TRUE)

# Funktion zur Berechnung von Kappa nur für ausgewählte Spalten und Zeilen
calculate_kappa_for_selected <- function(columns, label_suffix) {
  # Berechnung für die ausgewählten Spalten (Extract oder Comparison)
  all_test_values <- unlist(retest_merged_data[paste0(columns, "_test")])
  all_retest_values <- unlist(retest_merged_data[paste0(columns, "_retest")])
  
  # Berechne Cohen's Kappa für die aktuellen Spalten und speichere die Ergebnisse
  kappa_result <- cohen.kappa(cbind(all_test_values, all_retest_values))
  data.frame(Category = label_suffix, Kappa = kappa_result$kappa)
}

# Berechnung für Extraktions-Items
retest_kappa_extraction <- calculate_kappa_for_selected(extract_columns, "retest_extraction")

# Berechnung für Vergleichs-Items
retest_kappa_comparison <- calculate_kappa_for_selected(comp_columns, "retest_comparison")

# Zusammenfassung der Ergebnisse
retest_kappa_extraction_comparison <- rbind(retest_kappa_extraction, retest_kappa_comparison)

# Ergebnisse anzeigen
print(retest_kappa_extraction_comparison)
...
# Entfernen der Variablen pre_dcp_1_text_gpt und pap_dcp_1_text_gpt aus kappa_results, da diese numerische variablen sind...
kappa_results <- kappa_results[!(kappa_results$Variable %in% c("pre_dcp_1_text_gpt", "pap_dcp_1_text_gpt")), ]
...
##Kappa interrater gpt zu olmo  per ITEM/Category 2. ansatz
...
##Kappa interrater gpt zu olmo  per ITEM/Category 3. ansatz
```{r}
# Function to calculate Cohen's Kappa, z-value, and percentage agreement with detailed error handling
calculate_kappa_detailed <- function(data, column_gpt, column_olmo) {
  # Ensure both columns exist
  if(column_gpt %in% colnames(data) & column_olmo %in% colnames(data)) {
    # Extract columns
    gpt_col <- data[[column_gpt]]
    olmo_col <- data[[column_olmo]]
    
    # Check if all data in columns are NA
    if (all(is.na(gpt_col)) || all(is.na(olmo_col))) {
      return(c(NA, NA, NA, NA))
    }
    
    # Remove rows with NA values
    valid_data <- data[!is.na(gpt_col) & !is.na(olmo_col), ]
    
    # Calculate Cohen's Kappa using 'kappa2' from the 'irr' package
    kappa_result <- irr::kappa2(data.frame(valid_data[[column_gpt]], valid_data[[column_olmo]]), "unweighted")
    
    # Extract kappa value and p-value
    kappa_value <- kappa_result$value
    p_value <- kappa_result$p.value
    agreement_percentage <- mean(valid_data[[column_gpt]] == valid_data[[column_olmo]]) * 100
    
    # Calculate expected agreement (p_e)
    agreement_table <- table(valid_data[[column_gpt]], valid_data[[column_olmo]])
    n <- sum(agreement_table)  # Total number of pairs
    p_e <- sum(rowSums(agreement_table) * colSums(agreement_table)) / (n^2)
    ...
    return(c(kappa_value, p_value, z_value, agreement_percentage))
  } else {
    return(c(NA, NA, NA, NA))
  }
}

# Get all columns with _gpt and _olmo suffixes
gpt_columns_v3 <- grep("_gpt$", colnames(data_gpt_olmo_processed), value = TRUE)
olmo_columns_v3 <- gsub("_gpt$", "_olmo", gpt_columns_v3)

# Create a dataframe to store the results
kappa_results_v3 <- data.frame(Variable = gpt_columns_v3, Kappa = NA, P_Value = NA, Z_Value = NA, Percent_Agreement = NA)

# Loop over all _gpt and _olmo column pairs and calculate statistics
for(i in 1:length(gpt_columns_v3)) {
  kappa_stats <- calculate_kappa_detailed(data_gpt_olmo_processed, gpt_columns_v3[i], olmo_columns_v3[i])
  kappa_results_v3[i, 2:5] <- kappa_stats
}

# Display the results
print(kappa_results_v3)
...
## test nach normalverteilung über items
```{r}
# Shapiro-Wilk-Test zur Überprüfung auf Normalverteilung der Kappa-Werte
shapiro_test <- shapiro.test(kappa_results$Kappa)

# Shapiro-Wilk-Test-Ergebnisse anzeigen
cat("Shapiro-Wilk-Test für Kappa-Werte:\n")
print(shapiro_test)

# Normalverteilungsgrafik erstellen
# Histogramm mit angepasster Normalverteilungskurve
hist(kappa_results$Kappa, breaks = 40, prob = TRUE, 
     main = "Histogramm der Kappa-Werte mit Normalverteilungskurve", 
     xlab = "Kappa-Wert", ylab = "Dichte")
curve(dnorm(x, mean = mean(kappa_results$Kappa), sd = sd(kappa_results$Kappa)), 
      add = TRUE, col = "blue", lwd = 2)

# Q-Q-Plot zur Überprüfung der Normalverteilung
qqnorm(kappa_results$Kappa, main = "Q-Q-Plot der Kappa-Werte")
qqline(kappa_results$Kappa, col = "blue", lwd = 2)
...
if("Kappa" %in% colnames(kappa_results)) {
  # Berechne den Durchschnitt der Spalte Kappa und runde auf 2 Nachkommastellen
  Average_Kappa_over_categories <- round(mean(kappa_results$Kappa, na.rm = TRUE), 2)
  
  # Berechne die Standardabweichung der Spalte Kappa und runde auf 2 Nachkommastellen
  SD_Kappa_over_categories <- round(sd(kappa_results$Kappa, na.rm = TRUE), 2)
  
  # Berechne den Bereich der Spalte Kappa und runde auf 2 Nachkommastellen
  Range_Kappa_over_categories <- round(range(kappa_results$Kappa, na.rm = TRUE), 2)
  
  # Ausgabe der Ergebnisse
  cat("Durchschnittlicher Kappa-Wert über alle Kategorien:", Average_Kappa_over_categories, "\n")
  cat("Standardabweichung des Kappa-Werts über alle Kategorien:", SD_Kappa_over_categories, "\n")
  cat("Bereich des Kappa-Werts über alle Kategorien:", Range_Kappa_over_categories[1], "-", Range_Kappa_over_categories[2], "\n")
} else {
...
## Average percent agreement over items
```{r}
# Überprüfen, ob die Spalte "Percent_Agreement" im Datensatz kappa_results existiert
if("Percent_Agreement" %in% colnames(kappa_results)) {
  # Berechne den Durchschnitt der Spalte Percent_Agreement und runde auf 2 Nachkommastellen
  Average_Percent_Agreement_over_categories <- round(mean(kappa_results$Percent_Agreement, na.rm = TRUE), 2)
  
  # Berechne die Standardabweichung der Spalte Percent_Agreement und runde auf 2 Nachkommastellen
  SD_Percent_Agreement_over_categories <- round(sd(kappa_results$Percent_Agreement, na.rm = TRUE), 2)
  
  # Berechne den Bereich der Spalte Percent_Agreement und runde auf 2 Nachkommastellen
  Range_Percent_Agreement_over_categories <- round(range(kappa_results$Percent_Agreement, na.rm = TRUE), 2)
  
  # Ausgabe der Ergebnisse
  cat("Durchschnittlicher Percent_Agreement-Wert über alle Kategorien:", Average_Percent_Agreement_over_categories, "\n")
  cat("Standardabweichung des Percent_Agreement-Werts über alle Kategorien:", SD_Percent_Agreement_over_categories, "\n")
  cat("Bereich des Percent_Agreement-Werts über alle Kategorien:", Range_Percent_Agreement_over_categories[1], "-", Range_Percent_Agreement_over_categories[2], "\n")
} else {
  stop("Die Spalte 'Percent_Agreement' existiert nicht im Datensatz kappa_results!")
}
...
## Kappa interrater gpt zu olmo  per category 4. ansatz inkl sensitivity,specifity precision per item
```{r}
# Spalten mit _gpt und _olmo suffix finden
gpt_columns_per_item_v4 <- grep("_gpt$", colnames(data_gpt_olmo_processed), value = TRUE)
olmo_columns_per_item_v4 <- gsub("_gpt$", "_olmo", gpt_columns_per_item_v4)

# Überprüfen, ob die entsprechenden _olmo-Spalten existieren
valid_gpt_columns_per_item_v4 <- gpt_columns_per_item_v4[olmo_columns_per_item_v4 %in% colnames(data_gpt_olmo_processed)]
valid_olmo_columns_per_item_v4 <- olmo_columns_per_item_v4[olmo_columns_per_item_v4 %in% colnames(data_gpt_olmo_processed)]

# Entfernen der Variablen pre_dcp_1_text_gpt und pap_dcp_1_text_gpt, da diese nummerisch sind...
excluded_items <- c("pre_dcp_1_text_gpt", "pap_dcp_1_text_gpt")
valid_gpt_columns_per_item_v4 <- valid_gpt_columns_per_item_v4[!valid_gpt_columns_per_item_v4 %in% excluded_items]
valid_olmo_columns_per_item_v4 <- valid_olmo_columns_per_item_v4[!valid_olmo_columns_per_item_v4 %in% gsub("_gpt$", "_olmo", excluded_items)]

# Sicherstellen, dass nur gültige Paare verwendet werden
if(length(valid_gpt_columns_per_item_v4) == 0) {
...
kappa_results_per_item_v4 <- data.frame(Item = character(),
                                        Kappa = numeric(),
                                        P_Value = numeric(),
                                        Z_Value = numeric(),
                                        Percent_Agreement = numeric(),
                                        Sensitivity_n = numeric(),
                                        Specificity_n = numeric(),
                                        Precision_n = numeric(),
                                        stringsAsFactors = FALSE)

# Berechnung für jede Spalte einzeln
for(i in 1:length(valid_gpt_columns_per_item_v4)) {
  gpt_column_per_item_v4 <- data_gpt_olmo_processed[[valid_gpt_columns_per_item_v4[i]]]
  olmo_column_per_item_v4 <- data_gpt_olmo_processed[[valid_olmo_columns_per_item_v4[i]]]
  
  # Kombiniere GPT und OLMO Spalten für den Vergleich
  combined_data_per_item_v4 <- data.frame(GPT = gpt_column_per_item_v4,
                                          OLMO = olmo_column_per_item_v4)

  # Entferne NA-Werte
  combined_data_per_item_v4 <- na.omit(combined_data_per_item_v4)
  
  # Berechne Cohen's Kappa
  kappa_result_per_item_v4 <- irr::kappa2(combined_data_per_item_v4, "unweighted")
  
  # Berechne den Prozentsatz der Übereinstimmung
  percent_agreement_per_item_v4 <- sum(combined_data_per_item_v4$GPT == combined_data_per_item_v4$OLMO) / nrow(combined_data_per_item_v4) * 100
  
  # Berechnung der Confusion Matrix (y, n, x Kategorien)
  confusion_matrix <- table(combined_data_per_item_v4$GPT, combined_data_per_item_v4$OLMO)
  
  # Sicherstellen, dass "n" in der Matrix vorhanden ist
  if("n" %in% rownames(confusion_matrix) && "n" %in% colnames(confusion_matrix)) {
    TN <- confusion_matrix["n", "n"]
  } else {
    TN <- 0
  }

  # Berechne False Positives (FP) und False Negatives (FN) für "n"
  FP <- if("n" %in% rownames(confusion_matrix)) sum(confusion_matrix["n", ]) - TN else 0
  FN <- if("n" %in% colnames(confusion_matrix)) sum(confusion_matrix[, "n"]) - TN else 0
  
  # Berechne True Positives (TP) als restliche Übereinstimmungen
  TP <- sum(confusion_matrix) - (TN + FP + FN)

  # Berechne Sensitivity (n), Specificity (n) und Precision (n)
  sensitivity_n <- if((TN + FN) > 0) TN / (TN + FN) else NA
  specificity_n <- if((TP + FP) > 0) TP / (TP + FP) else NA
  precision_n <- if((TP + FN) > 0) TP / (TP + FN) else NA
  
  # Ergebnisse in den DataFrame hinzufügen
  kappa_results_per_item_v4 <- rbind(kappa_results_per_item_v4, data.frame(Item = valid_gpt_columns_per_item_v4[i],
                                                                           Kappa = kappa_result_per_item_v4$value,
                                                                           P_Value = kappa_result_per_item_v4$p.value,
                                                                           Z_Value = kappa_result_per_item_v4$statistic,
                                                                           Percent_Agreement = percent_agreement_per_item_v4,
                                                                           Sensitivity_n = sensitivity_n,
                                                                           Specificity_n = specificity_n,
...
# Ergebnisse anzeigen
print(kappa_results_per_item_v4)



write.xlsx(kappa_results_per_item_v4, "../Data/processed/kappa_results_per_item_v4.xlsx")
...
# Erstelle einen DataFrame, um die Ergebnisse für jede Spalte zu speichern
ac1_results_per_item_v4 <- data.frame(Item = character(),
                                      AC1 = numeric(),
                                      Percent_Agreement = numeric(),
                                      Sensitivity_n = numeric(),
                                      Specificity_n = numeric(),
...
for(i in 1:length(valid_gpt_columns_per_item_v4)) {
  ...
  # Erstelle die Häufigkeitstabelle für GWET's AC1
  agreement_table <- table(combined_data_per_item_v4$GPT, combined_data_per_item_v4$OLMO)

  # Berechne GWET's AC1
  ac1_result_per_item_v4 <- gwet.ac1.raw(agreement_table)
  ...
  # Ergebnisse in den DataFrame hinzufügen
  ac1_results_per_item_v4 <- rbind(ac1_results_per_item_v4, data.frame(Item = valid_gpt_columns_per_item_v4[i],
                                                                        AC1 = ac1_result_per_item_v4$est,
                                                                        Percent_Agreement = percent_agreement_per_item_v4,
                                                                        Sensitivity_n = sensitivity_n,
                                                                        Specificity_n = specificity_n,
                                                                        Precision_n = precision_n))
...
# Entfernen der Variablen pre_dcp_1_text_gpt und pap_dcp_1_text_gpt, da diese nummerisch sind
excluded_items <- c("pre_dcp_1_text_gpt", "pap_dcp_1_text_gpt")
valid_gpt_columns <- valid_gpt_columns[!valid_gpt_columns %in% excluded_items]
valid_olmo_columns <- valid_olmo_columns[!valid_olmo_columns %in% gsub("_gpt$", "_olmo", excluded_items)]
...
# Kombiniere alle gültigen GPT und OLMO Spalten in eine lange Liste
all_gpt_values <- unlist(data_gpt_olmo_processed[valid_gpt_columns])
all_olmo_values <- unlist(data_gpt_olmo_processed[valid_olmo_columns])

# Sicherstellen, dass die Kategorien konsistent sind (3 Kategorien z.B. Y, N, X)
# Umwandlung in Faktoren, falls die Werte nicht schon Faktoren sind
all_gpt_values <- factor(all_gpt_values, levels = c("Y", "N", "X"))
all_olmo_values <- factor(all_olmo_values, levels = c("Y", "N", "X"))

# Erstelle eine Häufigkeitstabelle für GWET's AC1
combined_data <- data.frame(GPT = all_gpt_values, OLMO = all_olmo_values)
combined_data <- na.omit(combined_data)
agreement_table <- table(combined_data$GPT, combined_data$OLMO)

# Berechne GWET's AC1 über alle Items
ac1_overall <- gwet.ac1.raw(agreement_table)

# Extrahiere den Schätzwert und zeige das Ergebnis an
cat("GWET's AC1 über alle Items:", ac1_overall$est$coeff.val, "\n")

# Vollständiges Ergebnis von GWET's AC1 anzeigen
print(ac1_overall)
```
...
## interrater kappa per prompt version
...
### siginifikanztest unterscheid promptversion a zu d
```{r}
# Funktion zur Berechnung des Fisher-Z-Werts für den Unterschied zwischen zwei Kappa-Werten
calculate_z_difference <- function(kappa1, se_kappa1, kappa2, se_kappa2) {
  if (!is.na(se_kappa1) & !is.na(se_kappa2)) {
    z_diff <- (kappa1 - kappa2) / sqrt(se_kappa1^2 + se_kappa2^2)
    p_value <- 2 * (1 - pnorm(abs(z_diff)))  # zweiseitiger Test
    return(c(z_diff, p_value))
  } else {
    return(c(NA, NA))  # Kein Vergleich möglich, falls SE nicht berechnet werden kann
  }
}

# Definiere die Versionen A und D für den Vergleich
version_compare <- c("A", "D")

# Speichern der Kappa-Ergebnisse für A und D
kappa_results_z <- list()

for (version_PromptV in version_compare) {
  # Filtere die Zeilen für die aktuelle Version
  version_data_PromptV <- data_gpt_olmo_processed[data_gpt_olmo_processed$prompt_version == version_PromptV, ]
  ...
  kappa_result_PromptV <- irr::kappa2(combined_data_PromptV, "unweighted")
  n_PromptV <- nrow(combined_data_PromptV)
  
  p_e_PromptV <- sum(rowSums(table(combined_data_PromptV$GPT, combined_data_PromptV$OLMO)) * colSums(table(combined_data_PromptV$GPT, combined_data_PromptV$OLMO))) / (n_PromptV^2)
  p_0_PromptV <- sum(combined_data_PromptV$GPT == combined_data_PromptV$OLMO) / n_PromptV
  ...
  # Speichern der Kappa-Werte und Standardfehler für Vergleich
  kappa_results_z[[version_PromptV]] <- list(kappa = kappa_result_PromptV$value, se = se_kappa_PromptV)
}

# Berechnen des Z-Werts und p-Werts für den Unterschied zwischen A und D
z_diff_results <- calculate_z_difference(kappa_results_z[["A"]]$kappa, kappa_results_z[["A"]]$se,
                                         kappa_results_z[["D"]]$kappa, kappa_results_z[["D"]]$se)

cat("\nUnterschied zwischen Cohen's Kappa für Version A und D:\n")
cat("Z-Wert:", z_diff_results[1], "\n")
cat("p-Wert:", z_diff_results[2], "\n")
...
###signifikanztest unterschied extraction comparison steigers z
```{r}
# Berechne den Unterschied zwischen den Kappa-Werten für Extraction und Comparison
z_diff_results <- calculate_z_difference(kappa_result_extract$value, se_kappa_extract,
                                         kappa_result_comp$value, se_kappa_comp)

cat("\nUnterschied zwischen Kappa für Extraction und Comparison:\n")
cat("Z-Wert:", z_diff_results[1], "\n")
cat("p-Wert:", z_diff_results[2], "\n")
...
# interrater Kappa NUR Y & N 

## interrater kappa y_n all categories
...
# NEW / MINIMAL VERSION (VE)
## interrater kappa new per category
```{r}
...
results_ve <- data.frame(Variable_ve = character(), 
                         ...
                         CI_Lower_ve = numeric(),  # Konfidenzintervall untere Grenze
                         CI_Upper_ve = numeric(),  # Konfidenzintervall obere Grenze
                         Accuracy_ve = numeric(),
                         Sensitivity_ve = numeric(),
                         Specificity_ve = numeric(),
                         N_Count_Olmo = numeric(),
                         N_Count_GPT = numeric(),
                         Y_Count_Olmo = numeric(),
                         Y_Count_GPT = numeric(),
                         stringsAsFactors = FALSE)
...
for (variable in variables_to_compare_ve) {
  ...
  # Berechne den Standardfehler für Cohen's Kappa
  se_kappa_ve <- sqrt((1 - kappa_value_ve^2) / n_comparisons_ve)
  
  # Berechne das 95%-Konfidenzintervall (z-Wert von 1.96 für 95%)
  ci_lower_ve <- kappa_value_ve - 1.96 * se_kappa_ve
  ci_upper_ve <- kappa_value_ve + 1.96 * se_kappa_ve
  
  # Erstelle Confusion-Matrix
  confusion_matrix <- table(comparison_data[[gpt_column]], comparison_data[[olmo_column]])
  true_positive <- confusion_matrix[2, 2]
  true_negative <- confusion_matrix[1, 1]
  false_positive <- confusion_matrix[1, 2]
  false_negative <- confusion_matrix[2, 1]

  # Berechne Genauigkeit, Spezifität und Sensitivität
  accuracy_ve <- (true_positive + true_negative) / n_comparisons_ve
  sensitivity_ve <- true_positive / (true_positive + false_negative)
  specificity_ve <- true_negative / (true_negative + false_positive)
  
  # Zähle die Anzahl der "N"- und "Y"-Werte in den Spalten von GPT und Olmo
  n_count_olmo <- sum(comparison_data[[olmo_column]] == "n", na.rm = TRUE)
  n_count_gpt <- sum(comparison_data[[gpt_column]] == "n", na.rm = TRUE)
  y_count_olmo <- sum(comparison_data[[olmo_column]] == "y", na.rm = TRUE)
  y_count_gpt <- sum(comparison_data[[gpt_column]] == "y", na.rm = TRUE)
  ...
  results_ve <- rbind(results_ve, data.frame(Variable_ve = variable, 
                                             ...
                                             CI_Lower_ve = ci_lower_ve,  
                                             CI_Upper_ve = ci_upper_ve,
                                             Accuracy_ve = accuracy_ve,
                                             Sensitivity_ve = sensitivity_ve,
                                             Specificity_ve = specificity_ve,
                                             N_Count_Olmo = n_count_olmo,
                                             N_Count_GPT = n_count_gpt,
                                             Y_Count_Olmo = y_count_olmo,
                                             Y_Count_GPT = y_count_gpt))
}
...
##confusion matrix mit unterschiedlicher Reiehnfolge und unterschiedlichem positive Class....




```{r}
# Ändere die Reihenfolge der Levels
combined_data_ve_total$Olmo_Rating_ve_total <- factor(combined_data_ve_total$Olmo_Rating_ve_total, levels = c("y", "n"))
combined_data_ve_total$GPT_Rating_ve_total <- factor(combined_data_ve_total$GPT_Rating_ve_total, levels = c("y", "n"))

# Erstellen der Confusion Matrix mit neuer Reihenfolge
conf_matrix_ve <- table(combined_data_ve_total$Olmo_Rating_ve_total, combined_data_ve_total$GPT_Rating_ve_total)

# Berechnung der Metriken mit spezifischer positiver Klasse
confusion_summary_ve <- confusionMatrix(conf_matrix_ve, positive = "y")
...
## confusion matrix VE new v2
```{r}
# Definiere die zu vergleichenden Variablenpaare mit 've_new'
variables_to_compare_ve_new <- c("match_dv_0", "match_dv_1", "match_dv_2", "match_dv_3",
                                 "match_dcp_1", "match_dcp_2", "match_sm_1", "match_sm_2", "match_sm_3")

# Liste zur Speicherung der kombinierten Ratings mit 've_new'
combined_data_ve_new <- data.frame(GPT_Rating_ve_new = character(), 
                                   Olmo_Rating_ve_new = character(), 
                                   stringsAsFactors = FALSE)

# Kombinieren der Daten aus allen relevanten Variablenpaaren
for (variable in variables_to_compare_ve_new) {
  # Definiere die Spaltennamen für GPT und Olmo
  gpt_column <- paste0(variable, "_gpt")
  olmo_column <- paste0(variable, "_olmo")
  
  # Selektiere die relevanten Spalten und entferne Zeilen mit NA-Werten nur in diesen Spalten
  comparison_data_ve_new <- data_gpt_ve_total_filtered %>%
    select(gpt_column, olmo_column) %>%
    filter(!is.na(.[[gpt_column]]) & !is.na(.[[olmo_column]])) %>%
    rename(GPT_Rating_ve_new = gpt_column, Olmo_Rating_ve_new = olmo_column)
  
  # Füge die gefilterten Daten zum kombinierten Datensatz hinzu
  combined_data_ve_new <- rbind(combined_data_ve_new, comparison_data_ve_new)
}

# Erstellen der Konfusionsmatrix mit Y/N Kategorien für die kombinierten Ratings mit 've_new'
conf_matrix_ve_new <- table(combined_data_ve_new$Olmo_Rating_ve_new, combined_data_ve_new$GPT_Rating_ve_new)

# Ausgabe der Konfusionsmatrix mit Y/N Kategorien
print("Confusion Matrix (ve_new):")
print(conf_matrix_ve_new)

# Berechnung der Summe der Zeilen und Spalten
row_sums_ve_new <- rowSums(conf_matrix_ve_new)  # Summe über die Zeilen (horizontal)
col_sums_ve_new <- colSums(conf_matrix_ve_new)  # Summe über die Spalten (vertikal)
n_total_ve_new <- sum(conf_matrix_ve_new)       # Gesamtsumme

# Erstelle eine erweiterte Konfusionsmatrix mit Summen und 've_new'
conf_matrix_with_sums_ve_new <- rbind(conf_matrix_ve_new, "Sum_ve_new" = col_sums_ve_new)  # Spaltensummen als neue Zeile hinzufügen
conf_matrix_with_sums_ve_new <- cbind(conf_matrix_with_sums_ve_new, "Sum_ve_new" = c(row_sums_ve_new, n_total_ve_new))  # Zeilensummen und Gesamtsumme hinzufügen

# Ausgabe der erweiterten Konfusionsmatrix mit Summen
print("Confusion Matrix with Sums (ve_new):")
print(conf_matrix_with_sums_ve_new)

# Anzeige im APA-Stil
library(kableExtra)
kable(conf_matrix_with_sums_ve_new, format = "html", caption = "Confusion Matrix (ve_new) with Row and Column Sums (APA style)") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"), full_width = F) %>%
  row_spec(0, bold = TRUE, extra_css = "border-bottom: 1px solid black;") %>%
  row_spec(nrow(conf_matrix_with_sums_ve_new), extra_css = "border-top: 1px solid black;")
...
if("kappa_value" %in% colnames(results_peritem)) {
  # Berechne den Durchschnitt der Spalte kappa_value und runde auf 2 Nachkommastellen
  Average_Kappa_over_psp <- round(mean(results_peritem$kappa_value, na.rm = TRUE), 2)
  
  # Berechne die Standardabweichung der Spalte kappa_value und runde auf 2 Nachkommastellen
  SD_Kappa_over_psp <- round(sd(results_peritem$kappa_value, na.rm = TRUE), 2)
  
  # Berechne den Bereich der Spalte kappa_value und runde auf 2 Nachkommastellen
  Range_Kappa_over_psp <- round(range(results_peritem$kappa_value, na.rm = TRUE), 2)
  
  # Ausgabe der Ergebnisse
  cat("Durchschnittlicher Kappa-Wert über alle psp:", Average_Kappa_over_psp, "\n")
  cat("Standardabweichung des Kappa-Werts über alle psp:", SD_Kappa_over_psp, "\n")
  cat("Bereich des Kappa-Werts über alle psp:", Range_Kappa_over_psp[1], "-", Range_Kappa_over_psp[2], "\n")
} else {
...
hist(results_peritem$kappa_value, main = "Histogramm der Kappa-Werte", xlab = "Kappa-Wert", breaks = 50)


# Shapiro-Wilk-Test zur Überprüfung auf Normalverteilung der Kappa-Werte
shapiro_test <- shapiro.test(results_peritem$kappa_value)
...
# interrater kappa per EXPERIMENT TYPE
```{r}
# Funktion zur Berechnung des Konfidenzintervalls für Kappa
calculate_kappa_confidence_interval <- function(kappa_value, n, alpha = 0.05) {
  # Standardfehler von Kappa
...
## differenztest steigers z per intterater experiment type
```{r}
# Funktion zur Berechnung des Z-Tests für den Unterschied zwischen zwei Kappa-Werten
calculate_z_difference <- function(kappa1, se_kappa1, kappa2, se_kappa2) {
  if (!is.na(se_kappa1) & !is.na(se_kappa2)) {
    z_diff <- (kappa1 - kappa2) / sqrt(se_kappa1^2 + se_kappa2^2)
    p_value <- 2 * (1 - pnorm(abs(z_diff)))  # Zweiseitiger Test
...
# Spezifische Experimenttypen für den Vergleich
type1 <- "Effect"
type2 <- "Association"

# Funktion zur Berechnung des Kappa und SE für einen spezifischen Experimenttyp
calculate_kappa_for_type <- function(type) {
  type_data <- data_gpt_olmo_processed[data_gpt_olmo_processed$type == type, ]
  combined_data <- data.frame()
  
  for(i in 1:length(valid_gpt_columns_experimenttype)) {
    combined_data <- rbind(combined_data, 
                           data.frame(GPT = type_data[[valid_gpt_columns_experimenttype[i]]],
                                      OLMO = type_data[[valid_olmo_columns_experimenttype[i]]]))
  }
  ...
  if (nrow(combined_data) > 0) {
    ...
    standard_error <- sqrt((1 - kappa_value^2) / n)
    return(list(kappa = kappa_value, se = standard_error))
  } else {
    return(list(kappa = NA, se = NA))
  }
}

# Berechnung für die beiden Experimenttypen
kappa_results_type1 <- calculate_kappa_for_type(type1)
kappa_results_type2 <- calculate_kappa_for_type(type2)

# Berechnung des Z-Werts und p-Werts für den Unterschied zwischen den beiden Kappa-Werten
z_diff_results <- calculate_z_difference(kappa_results_type1$kappa, kappa_results_type1$se,
                                         kappa_results_type2$kappa, kappa_results_type2$se)

cat("Unterschied zwischen Kappa für", type1, "und", type2, ":\n")
cat("Z-Wert:", z_diff_results[1], "\n")
cat("p-Wert:", z_diff_results[2], "\n")
...
#protoypische /häufigste Antwort

## protoype olmo
```{r}
# Erstellen des neuen Datensatzes `data_prototype`
data_prototype <- data.frame(Variable = character(),
                             Prototype = character(),
                             Prototype_Percentage = numeric(),
                             stringsAsFactors = FALSE)

# Schleife über jede Spalte in `data_gpt_olmo_processed`
for (col_name in names(data_gpt_olmo_processed)) {
  # Prüfen, ob die Spalte gültige Werte enthält
  if (sum(!is.na(data_gpt_olmo_processed[[col_name]])) > 0) {
    # Ermitteln der häufigsten Antwort und deren Häufigkeit
    prototype_value <- as.character(names(sort(table(data_gpt_olmo_processed[[col_name]]), decreasing = TRUE)[1]))
    prototype_count <- max(table(data_gpt_olmo_processed[[col_name]]))
    
    # Berechnen des Prozentsatzes
    prototype_percentage <- (prototype_count / nrow(data_gpt_olmo_processed)) * 100
  } else {
    # Falls die Spalte nur NAs enthält, setzen wir Werte auf NA
    prototype_value <- NA
    prototype_percentage <- NA
  }
  
  # Hinzufügen der Werte zu `data_prototype`
  data_prototype <- rbind(data_prototype, 
...
# Filtere data_prototype, um nur Zeilen mit Prototype-Werten 'y', 'n' oder 'x' zu behalten
data_prototype <- subset(data_prototype, Prototype %in% c("y", "n", "x"))
...
## mergen von data_protoype und retest_kappa summaryv2
```{r}

# Anzeigen der einzigartigen Werte von 'Column' in retest_kappa_summary_v2
unique(retest_kappa_summary_v2$Column)

# Anzeigen der einzigartigen Werte von 'Variable_clean' in data_prototype (nach Bearbeitung)
data_prototype$Variable_clean <- gsub("_gpt$", "_comp", data_prototype$Variable)
unique(data_prototype$Variable_clean)

# Optional: Zeige Zeilen, die keine Übereinstimmung haben
anti_join(data_prototype, retest_kappa_summary_v2, by = c("Variable_clean" = "Column"))

# Durchführung des Merges nach Überprüfung
data_retest_prototype <- retest_kappa_summary_v2 %>%
  inner_join(data_prototype, by = c("Column" = "Variable_clean"))

# Ergebnis anzeigen
head(data_retest_prototype)

```



## korrelation zw protoype percentage und kappa(retest)
```{r}
# Korrelation berechnen
correlation_result <- cor.test(data_retest_prototype$Kappa, data_retest_prototype$Prototype_Percentage, method = "pearson")

# Ergebnisse im APA-Format ausgeben
cat(sprintf("r(%d) = %.2f, p = %.3f, 95%% CI [%.2f, %.2f]",
            correlation_result$parameter,               # Freiheitsgrade (df)
            correlation_result$estimate,               # Korrelationskoeffizient (r)
            correlation_result$p.value,                # p-Wert
            correlation_result$conf.int[1],            # Untere Grenze des Konfidenzintervalls
            correlation_result$conf.int[2]))           # Obere Grenze des Konfidenzintervalls
...
## mergen von data_protoype und kappa_results
```{r}
# Sicherstellen, dass die Spalten 'Variable' in beiden Datensätzen als Zeichenketten behandelt werden
data_prototype$Variable <- as.character(data_prototype$Variable)
kappa_results$Variable <- as.character(kappa_results$Variable)

# Merging der beiden Datensätze direkt anhand der Spalte 'Variable'
data_interrater_prototype <- merge(
  data_prototype, 
  kappa_results, 
  by = "Variable", 
  all = TRUE
...
# Entfernen aller Zeilen, bei denen die Variable `Variable` auf '_olmo' oder '_date' endet
data_interrater_prototype <- data_interrater_prototype[!grepl("(_olmo|_date)$", data_interrater_prototype$Variable), ]

# Ergebnis anzeigen
head(data_interrater_prototype)
...
## korrelation zw protoype percentage und kappa(interrater) sowie agreement
```{r}

## Berechnung der Korrelation zwischen Kappa und Prototype_Percentage
correlation_kappa_prototype <- cor.test(data_interrater_prototype$Kappa, data_interrater_prototype$Prototype_Percentage)

# Ausgabe im APA-Format für Kappa und Prototype_Percentage
cat(sprintf("Kappa und Prototype_Percentage: r(%d) = %.2f, p = %.3f, 95%% CI [%.2f, %.2f]\n",
            correlation_kappa_prototype$parameter,
            correlation_kappa_prototype$estimate,
            correlation_kappa_prototype$p.value,
            correlation_kappa_prototype$conf.int[1],
            correlation_kappa_prototype$conf.int[2]))

# Berechnung der Korrelation zwischen Percent_Agreement und Prototype_Percentage
correlation_percent_prototype <- cor.test(data_interrater_prototype$Percent_Agreement, data_interrater_prototype$Prototype_Percentage)

# Ausgabe im APA-Format für Percent_Agreement und Prototype_Percentage
cat(sprintf("Percent_Agreement und Prototype_Percentage: r(%d) = %.2f, p = %.3f, 95%% CI [%.2f, %.2f]\n",
            correlation_percent_prototype$parameter,
            correlation_percent_prototype$estimate,
            correlation_percent_prototype$p.value,
            correlation_percent_prototype$conf.int[1],
            correlation_percent_prototype$conf.int[2]))

# Berechnung der Korrelation zwischen Percent_Agreement und Kappa
correlation_percent_kappa <- cor.test(data_interrater_prototype$Percent_Agreement, data_interrater_prototype$Kappa)

# Ausgabe im APA-Format für Percent_Agreement und Kappa
cat(sprintf("Percent_Agreement und Kappa: r(%d) = %.2f, p = %.3f, 95%% CI [%.2f, %.2f]",
            correlation_percent_kappa$parameter,
            correlation_percent_kappa$estimate,
            correlation_percent_kappa$p.value,
            correlation_percent_kappa$conf.int[1],
            correlation_percent_kappa$conf.int[2]))
...
pre_counts <- as.data.frame(table(na.omit(data_gpt_olmo_numeric$uebereinstimmend_pre)))
colnames(pre_counts) <- c("Kategorie", "Anzahl")
...
pap_counts <- as.data.frame(table(na.omit(data_gpt_olmo_numeric$uebereinstimmend_pap)))
colnames(pap_counts) <- c("Kategorie", "Anzahl")
pap_counts$Prozentsatz <- round(100 * pap_counts$Anzahl / sum(pap_counts$Anzahl), 2)

# Zusammenführen der Kategorien 
combined_counts <- merge(pre_counts, pap_counts, by = "Kategorie", all = TRUE, suffixes = c("_PRE", "_PAP"))

# Berechnung der Gesamtsummen ohne 
combined_counts$Anzahl_Gesamt <- combined_counts$Anzahl_PRE + combined_counts$Anzahl_PAP
combined_counts$Prozentsatz_Gesamt <- round(100 * combined_counts$Anzahl_Gesamt / sum(combined_counts$Anzahl_Gesamt), 2)

# Berechnung der durchschnittlichen Abweichung für abweichung_pre ohne 
durchschnittliche_abweichung_pre <- mean(data_gpt_olmo_numeric$abweichung_pre, na.rm = TRUE)

# Berechnung der durchschnittlichen Abweichung für abweichung_pap ohne 
durchschnittliche_abweichung_pap <- mean(data_gpt_olmo_numeric$abweichung_pap, na.rm = TRUE)

# Ausgabe der kombinierten Tabelle
cat("Kombinierte Tabelle für übereinstimmend_pre und übereinstimmend_pap - Absolute Zahlen und Prozentsätze (ohne NAs):\n")
...
# Promptdauer
```{r}
## Berechnung des Mittelwerts und der Standardabweichung für time_extraction und time_comparison, gerundet auf 2 Dezimalstellen
summary_stats <- data_gpt_olmo_processed %>%
  summarise(
    mean_time_extraction = round(mean(time_extraction, na.rm = TRUE), 2),
    sd_time_extraction = round(sd(time_extraction, na.rm = TRUE), 2),
    mean_time_comparison = round(mean(time_comparison, na.rm = TRUE), 2),
    sd_time_comparison = round(sd(time_comparison, na.rm = TRUE), 2)
  )
