...
Here i work with the dataset comparison_data_processed does it maybe make a difference because i have way more categories?!(y,n fxy etc)?
...
gwets_ac1_results_per_item <- data.frame(Item = character(),
                                         Gwet_AC1 = numeric(),
                                         Observed_Agreement = numeric(),
                                         Expected_Agreement = numeric(),
                                         stringsAsFactors = FALSE)

# Iterate through each valid GPT and OLMO column pair
for (i in seq_along(valid_gpt_columns_per_item_v3)) {
  gpt_column <- data_gpt_olmo_processed[[valid_gpt_columns_per_item_v3[i]]]
  olmo_column <- data_gpt_olmo_processed[[valid_olmo_columns_per_item_v3[i]]]

  # Combine GPT and OLMO columns for comparison, removing NAs
  combined_data <- na.omit(data.frame(GPT = gpt_column, OLMO = olmo_column))
  
  # Calculate observed agreement
  observed_agreement <- mean(combined_data$GPT == combined_data$OLMO)
  
  # Calculate category proportions for each rater
  categories <- unique(c(combined_data$GPT, combined_data$OLMO))
  rater1_props <- prop.table(table(combined_data$GPT))
  rater2_props <- prop.table(table(combined_data$OLMO))
  
  # Compute expected agreement based on Gwet's formula
  expected_agreement <- mean(
    sapply(categories, function(k) {
      p1 <- ifelse(!is.na(rater1_props[k]), rater1_props[k], 0)
      p2 <- ifelse(!is.na(rater2_props[k]), rater2_props[k], 0)
      2 * p1 * p2 + (1 - p1) * (1 - p2)
    })
  )
  
  # Calculate Gwet's AC1
  gwet_ac1 <- (observed_agreement - expected_agreement) / (1 - expected_agreement)
  
  # Append results to the DataFrame
  gwets_ac1_results_per_item <- rbind(gwets_ac1_results_per_item, 
                                      data.frame(Item = valid_gpt_columns_per_item_v3[i],
                                                 Gwet_AC1 = gwet_ac1,
...
# Display results
print(gwets_ac1_results_per_item)
...
# Iterate through each valid GPT and corresponding OLMO column pair
for (i in seq_along(valid_gpt_columns_per_item_v3)) {
  gpt_column <- data_gpt_olmo_processed[[valid_gpt_columns_per_item_v3[i]]]
  olmo_column <- data_gpt_olmo_processed[[valid_olmo_columns_per_item_v3[i]]]  # Correct column for comparison
  ...
  if (nrow(combined_data) > 0) {
    ...
    # Calculate expected agreement using Gwet's AC1 formula
    rater1_props <- prop.table(table(combined_data$GPT))
    rater2_props <- prop.table(table(combined_data$OLMO))
    categories <- unique(c(combined_data$GPT, combined_data$OLMO))
    
    # Diagnostic: Display proportions for categories
    print(paste("Item:", valid_gpt_columns_per_item_v3[i]))
    print("Rater1 Proportions:")
    print(rater1_props)
    print("Rater2 Proportions:")
    print(rater2_props)
    
    # Gwet's expected agreement: weighted calculation for each category
    expected_agreement <- mean(
      sapply(categories, function(k) {
        p1 <- ifelse(!is.na(rater1_props[k]), rater1_props[k], 0)
        p2 <- ifelse(!is.na(rater2_props[k]), rater2_props[k], 0)
    ...
    # Print expected agreement if it exceeds 1
    if (expected_agreement > 1) {
      print(paste("Warning: Expected Agreement > 1 for item", valid_gpt_columns_per_item_v3[i]))
    ...
    if (expected_agreement != 1) {
      ...
      gwet_ac1 <- NA  # AC1 is undefined if expected agreement is 1
    }
    ...
    # If no data in combined_data, append NA values
    gwets_ac1_results_per_item <- rbind(gwets_ac1_results_per_item, 
                                        data.frame(Item = valid_gpt_columns_per_item_v3[i],
                                                   Gwet_AC1 = NA,
...
for (version_PromptV in version_compare) {
  ...
  kappa_results[[version_PromptV]] <- list(kappa = kappa_result_PromptV$value, se = se_kappa_PromptV)
}

# Berechnen des Z-Werts und p-Werts für den Unterschied zwischen A und D
z_diff_results <- calculate_z_difference(kappa_results[["A"]]$kappa, kappa_results[["A"]]$se,
                                         kappa_results[["D"]]$kappa, kappa_results[["D"]]$se)
...
##signifikanztest unterschied extraction comparison
```{r}
...
# significant per intterater experiment/hypothese type
```{r}
...
```{r}
# Sicherstellen, dass die Spalten 'Variable' in beiden Datensätzen den gleichen Datentyp haben
data_prototype$Variable <- as.character(data_prototype$Variable)
kappa_results$Variable <- as.character(kappa_results$Variable)

# Entfernen von '_comp' in data_prototype und '_gpt' in kappa_results, um das Matching zu ermöglichen
data_prototype$Variable_clean <- sub("_comp$", "", data_prototype$Variable)
kappa_results$Variable_clean <- sub("_gpt$", "", kappa_results$Variable)

# Merging der beiden Datensätze anhand der 'Variable_clean' Spalte und die neue Datei data_interrater_prototype nennen
...
# Entfernen aller Zeilen, bei denen die Variable `Variable_clean` auf '_olmo' oder '_date' endet
data_interrater_prototype <- data_interrater_prototype[!grepl("(_olmo|date)$", data_interrater_prototype$Variable_clean), ]
