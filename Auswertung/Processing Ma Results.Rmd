...
data_olmo_selected_mutated <- data_olmo_selected_mutated %>% mutate(
    ...
    match_iec = ifelse(is.na(match_iec), "x", match_iec),
...
data_merged <- merge(data_gpt_finito, data_olmo_finito, by = "psp", suffixes = c("_gpt", "_olmo"))
...
# Neue DataFrame für die Vergleiche erstellen
comparison_results <- data_merged[ , "psp", drop=FALSE]

# Für jede Spalte den Vergleich durchführen
for (col in gpt_categories_alphabetic) {
  comparison_results[[col]] <- mapply(compare_columns, 
                                      data_merged[[paste0(col, "_gpt")]], 
                                      data_merged[[paste0(col, "_olmo")]])
}
