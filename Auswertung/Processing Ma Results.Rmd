...
Example 1: Variable Cleaning and Type Conversion
Task:
Clean the following variables by formatting their column names as if they were processed with the janitor package (all lowercase). Write the code to convert all these variables into numeric types. If a cell does not contain a numeric value, set it to "exclude."

Variables:
PreDCP_1_TEXT, PapDCP_1_TEXT, S_PRE_IV, S_Pre_DV, S_Pre_DCP, S_Pre_SMU, S_Pre_SIC, S_Pap_IV, S_Pap_DV, S_Pap_DCP, S_Pap_SMU, S_Pap_SIC, Prompt version a Strictness PREREG, Prompt version aStrictness Pubstud, Con_IV, Con_DV, Con_DCP, Con_SMU, Con_SIC, Global_Consistency_Esimate.
________________________________________
Example 2: Creating a New Dataset and Renaming Values
Task:
Create a new dataset data_results_finito based on the dataset data_results_filtered_clean_exclude_numeric.

In the column prompt_version, rename the values "v4" and "V4" to "D."
Add a new column master_exclude. Set its value to "include" for all rows, except for rows where the column date_gpt has an NA. In these cases, set the value to "exclude."
________________________________________
Example 3: Renaming and Cleaning Columns
Task:
Clean the following column names by converting them to lowercase as if they were processed with the janitor package:
PSP, Coder, PrePreMI1, PreMI1, PreIV1, CPreIV1, PreNIV1_1, PreNIV1_2, PreNIV1_3, PrePreMI2, PreMI2, PreIV2, CPreIV2, PreNIV2_1, PreNIV2_2, PreNIV2_3.

Apply these changes to the dataset data_olmo_raw_selected.
________________________________________
Example 4: Overview of Column Types
Task:
Is there a command in R that provides an overview of which variables in a dataset are stored as character and which are stored as numeric? Write the code to accomplish this task.
________________________________________
Example 5: Conditional Transformation of NA Values
Task:
Mutate the dataset data_olmo_raw_selected_mutated to replace missing values (NA) in the variables pre_pre_mi1, pre_mi1, pre_iv1, c_pre_iv1, pre_niv1_1, pre_niv1_2, pre_niv1_3 under the following conditions:

If pre_niv1_3 = "No, none of the elements", set the NA values in pre_niv1_1 and pre_niv1_2 to "n."
If either pre_niv1_1 or pre_niv1_2 contains the value "y," set the NA values in the other variable to "n."
Otherwise, replace all remaining NA values with "x."
________________________________________
Example 6: Refined Exception Handling
Task:
If only one of the two columns pre_niv1_1 or pre_niv1_2 contains the value "y," replace the missing values (NA) in the other column with "n." Ensure that no other conditions are overwritten by this logic.
________________________________________
Example 7: Step-by-Step Code Explanation
Task:
...
data_gpt_new_filtered <- data_gpt_new %>%
  select(   
    -prereg_vorhanden,
    -paper_vorhanden,
    -gpt_link_extraction,
...
#for new vE
gpt_new_frequencies_categories <- lapply(data_gpt_new_filtered[gpt_categories_alphabetic], table)
...
data_gpt_new_filtered_clean <- data_gpt_new_filtered %>%
  mutate_at(vars(one_of(gpt_categories_alphabetic)), clean_value)

gpt_new_frequencies_categories_cleaned <- lapply(data_gpt_new_filtered_clean[gpt_categories_alphabetic], table)

#gpt_frequencies_categories_cleaned

#gpt_new_frequencies_categories_cleaned
...
#new
data_gpt_new_filtered_clean_exclude <- data_gpt_new_filtered_clean %>%
  mutate_at(vars(one_of(gpt_categories_alphabetic)), replace_with_exclude)


# Check gpt frequencies after cleaning
gpt_frequencies_categories_cleaned_exclude <- lapply(data_gpt_filtered_clean_exclude[gpt_categories_alphabetic], table)

# Check new frequencies after cleaning
gpt_new_frequencies_categories_cleaned_exclude <- lapply(data_gpt_new_filtered_clean_exclude[gpt_categories_alphabetic], table)


#gpt_frequencies_categories_cleaned_exclude

#gpt_new_frequencies_categories_cleaned_exclude
```
...
# Convert gpt to numeric and replace non-numeric with 'exclude'
...
# Convert new to numeric and replace non-numeric with 'exclude'
data_gpt_new_filtered_clean_exclude_numeric <- data_gpt_new_filtered_clean_exclude %>%
  mutate_at(vars(one_of(gpt_categories_numeric)), 
            ~ifelse(is.na(as.numeric(.)), "exclude", as.numeric(.)))
...
#new
# Check new frequencies after cleaning
gpt_new_frequencies_categories_cleaned_exclude_numeric <- lapply(data_gpt_new_filtered_clean_exclude_numeric[gpt_categories_numeric], table)

#gpt_new_frequencies_categories_cleaned_exclude_numeric
...
#for new
# Ändere die Werte "v4" und "V4" in der Spalte prompt_version zu "D"
data_gpt_new_finito <- data_gpt_new_filtered_clean_exclude_numeric %>%
  mutate(prompt_version = ifelse(prompt_version %in% c("v4", "V4"), "D", prompt_version))

# Neue Spalte master_exclude hinzufügen und standardmäßig auf "include" setzen
data_gpt_new_finito <- data_gpt_new_finito %>%
  mutate(master_exclude = ifelse(is.na(date_gpt), "exclude", "include"))


# Erstellen des neuen Datensatzes
data_gpt_ve <- data_gpt_new_finito %>%
  # Filtern nach Zeilen, bei denen die Spalte 'prompt_version' den Wert 'E' hat
  filter(prompt_version == "E") %>%
  # Auswahl der gewünschten Spalten
  select(psp, title_pap, study_pap, date_gpt, prompt_version, test_retest,
         deviation_text_new, master_exclude, match_dv_0, match_dv_1,
         match_dv_2, match_dv_3, match_dcp_1, match_dcp_2,
         match_sm_1, match_sm_2, match_sm_3)

# Optional: Überprüfung der ersten Zeilen des neuen Datensatzes
head(data_gpt_ve)
...
olmo_selected_columns <- c(
  "psp",  "coder", "type" ,"pre_pre_mi1", "pre_mi1", "pre_iv1", "c_pre_iv1", "pre_niv1_1", 
...
#achtung bei iec repesktive iec 1 bedeutet eine leer gelassene zeile eventuell n ?!!!
### dcp/iec
...
# Auswahl der gewünschten Spalten aus dem Olmo-Datensatz
data_olmo_new_filtered <- data_olmo_finito %>%
  select(psp, match_dv_0, match_dv_1, match_dv_2, match_dv_3,
         match_dcp_1, match_dcp_2, match_sm_1, match_sm_2, match_sm_3)

# Merging von `data_gpt_ve` und `data_olmo_filtered` basierend auf der Spalte "psp"
data_gpt_ve_merged <- left_join(data_gpt_ve, data_olmo_new_filtered, by = "psp", suffix = c("_gpt", "_olmo"))

# Anpassen der Spalte 'master_exclude' basierend auf den Bedingungen für die "olmo"-Spalten
data_gpt_ve_merged <- data_gpt_ve_merged %>%
  mutate(master_exclude = if_else(rowSums(across(ends_with("_olmo"), ~ . == "x")) > 0, "exclude", "include"))

# Optional: Überprüfung der ersten Zeilen des neuen Datensatzes
head(data_gpt_ve_merged)
...
## remove master exclude in comparison 
...
## remove master exclude in  data_gpt_olmo_merged
...
## number before excluisone, for retest and for prompt versions...
...
```{r}
data_psp_filesize <- read_csv("../Data/raw/DATA_psp_filesize.csv") |>
  janitor::clean_names()
...
# Zunächst: Einen Überblick der Dateigrößen-Spalte anzeigen
summary(data_psp_filesize$size_kb)

# Histogramm der Verteilung der Dateigrößen (in KB)
ggplot(data_psp_filesize, aes(x = size_kb)) +
  geom_histogram(binwidth = 100, fill = "lightblue", color = "black") +
  labs(title = "Verteilung der Dateigrößen (in KB)", x = "Dateigröße (KB)", y = "Anzahl der Dateien") +
  theme_minimal()
...
```{r}
# Neuen Datensatz erstellen mit der Spalte 'psp' (Nummer aus dem Dateinamen)
data_psp_filesize_sum <- data_psp_filesize %>%
  # Extrahiere die Nummer aus dem Dateinamen
  mutate(psp = gsub("\\D", "", name)) %>%
  group_by(psp) %>%
  summarise(
    # Summe der Dateigrößen für das gesamte Paar (Prereg und Publikation)
    size_kb_sum = sum(size_kb),
    
    # Größe der Prereg-Datei extrahieren
    size_kb_prereg = sum(size_kb[grepl("Prereg.pdf", name)]),
    
    # Größe der Publikationsdatei (nicht "Prereg.pdf") extrahieren
    size_kb_pubstud = sum(size_kb[!grepl("Prereg.pdf", name)])
  )

# Ergebnis anzeigen
print(data_psp_filesize_sum)

# Histogramm der summierten Dateigrößen (in KB)
ggplot(data_psp_filesize_sum, aes(x = size_kb_sum)) +
  geom_histogram(binwidth = 100, fill = "blue", color = "black") +
  labs(title = "Verteilung der summierten Dateigrößen (in KB)", x = "Summierte Dateigröße (KB)", y = "Anzahl der Dateien") +
  theme_minimal()
...
## merge file size to the processed tatasets.

```{r}

# Umwandeln der 'psp'-Spalte in data_psp_filesize_sum in numerisch (double), um sie mit den anderen Datensätzen kompatibel zu machen
data_psp_filesize_sum <- data_psp_filesize_sum %>%
  mutate(psp = as.numeric(psp))

# Left Join des Datensatzes data_psp_filesize_sum mit comparison_data_processed
comparison_data_processed <- comparison_data_processed %>%
  left_join(data_psp_filesize_sum, by = "psp")

# Left Join des Datensatzes data_psp_filesize_sum mit data_gpt_olmo_processed
data_gpt_olmo_processed <- data_gpt_olmo_processed %>%
  left_join(data_psp_filesize_sum, by = "psp")

# Left Join des Datensatzes data_psp_filesize_sum mit data_gpt_ve_merged
data_gpt_ve_merged <- data_gpt_ve_merged %>%
  left_join(data_psp_filesize_sum, by = "psp")

# Optional: Anzeigen der Ergebnisse (hier ein Beispiel für die Ausgabe)
head(comparison_data_processed)
head(data_gpt_olmo_processed)
head(data_gpt_ve_merged)
...
write.xlsx(data_gpt_ve_merged, "../Data/processed/data_gpt_ve_processed.xlsx")
